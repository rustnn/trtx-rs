...
{
    static constexpr int32_t kVALUE = 29;
};
...
//!
struct TRT_DEPRECATED TimingCacheKey
{
    uint8_t data[16];
};
...
//! \deprecated Deprecated in TensorRT-RTX 1.2. Timing cache operations are no-ops in TensorRT-RTX.
//!
struct TRT_DEPRECATED TimingCacheValue
{
...
//! \deprecated Deprecated in TensorRT-RTX 1.2. Timing cache operations are no-ops in TensorRT-RTX.
//!
class TRT_DEPRECATED ITimingCache : public INoCopy
{
public:
...
//!
//! \brief Describes compute capability that an engine will be built for.
//!
...
public:
    ...
    //!
    //! \note Default number of compute capabilities is 0. If the new number of compute capabilities is less than the
    //! current number, setNbComputeCapabilities() removes elements beyond the new number, and not modifying the
    //! existing elements. If the new number of compute capabilities is greater than the current number,
    //! new elements are added to the end of the vector, and the new elements are set to ComputeCapability::kNONE.
    //!
    //! \param maxNbComputeCapabilities The maximum number of compute capabilities to set.
    //!
    //! \return True if successful, false otherwise
    //!
    ...
    //!
    //! \brief Get the number of compute capabilities
    //!
    //! \return The number of compute capabilities.
    //!
    ...
    //!
    //! \note If computeCapability is ComputeCapability::kCURRENT, the index must be 0, and the number of
    //! compute capabilities must be 1.
    //!
    //! \param computeCapability The compute capability to set.
    //! \param index The index at which to set the compute capability.
    //!
    //! \return True if successful, false otherwise
    //!
    ...
    //!
    //! \brief Get one compute capability for runtime execution.
    //!
    //! \param index The index of the compute capability to get.
    //!
    //! \return The compute capability at the specified index.
    //!
