...
//! enum EngineValidity
//! \brief Whether a TensorRT-RTX engine is likely to be valid on the current system.
//! \note The value is determined by only inspecting the header, so even an engine marked
//! kVALID or kSUBOPTIMAL may be invalid if the body has become corrupted.
//! \see IRuntime::getEngineValidity()
...
//! Maximum number of elements in EngineValidity enum. \see EngineValidity
template <>
struct impl::EnumMaxImpl<EngineValidity>
{
    static constexpr int32_t kVALUE = 3;
};

//! \enum EngineInvalidityDiagnostics
//! \brief Bitmask indicating the reason(s) why an engine is invalid.
//! \note There may be multiple reasons for engine invalidity, e.g. kVERSION_MISMATCH | kUNSUPPORTED_CC.
//! \see IRuntime::getEngineValidity()
...
public:
    ...
    //!
    //! \brief Get size of engine header in bytes.
    //!
    //! \details This represents the minimum size of the buffer to be passed to getEngineValidity().
    //! When checking the validity of large engine files, users may save time and memory by only
    //! loading the first getEngineHeaderSize() bytes of the engine file from the file system.
    //!
    ...
    //!
    //! \brief Check for engine validity by inspecting the serialized engine header.
    //!
    //! \param blob The start address of a memory buffer containing either a serialized engine or
    //! the header of a serialized engine.
    //!
    //! \param blobSize The size of the memory buffer in bytes.
    //!
    //! \param diagnostics Either nullptr or the address of a bitmask where to write diagnostic
    //!        information why the validity check failed.
    //!
    //! \return Whether the engine is valid, suboptimal or invalid, based on the header information.
    //!
    //! \important The buffer must have a length of at least getEngineHeaderSize() bytes.
    //!
    //! \important This function only checks the engine header, so it cannot guarantee that
    //! engine deserialization will succeed. For example, it cannot detect whether the body of
    //! the serialized engine has become corrupted.
    //!
    //! \details If a non-null pointer is passed as the diagnostics argument, *diagnostics will
    //! be set to 0 if the return value is EngineValidity::kVALID or EngineValidity::kSUBOPTIMAL.
    //! If the return value is EngineValidity::kINVALID, *diagnostics will contain a nonzero bitmask
    ...
    //!
    //! \brief Defer GPU weight allocation during deserializeCudaEngine().
    //!
    //! When set to true, subsequent deserializeCudaEngine() calls parse the engine plan
    //! and lay out runtime metadata, but skip allocating and copying the engine's weights
    //! onto the GPU. JIT compilation (createExecutionContext) succeeds against the host-resident
    //! weight metadata. Calling IExecutionContext::enqueueV3() or
    //! IExecutionContext::executeV2() returns false (without running inference)
    //! until ICudaEngine::loadWeights() or ICudaEngine::loadWeightsAsync() is
    //! called to copy the weights to the GPU.
    //!
    //! Defaults to false. Must be called BEFORE deserializeCudaEngine().
    //!
    //! \see ICudaEngine::loadWeights, ICudaEngine::loadWeightsAsync, ICudaEngine::areWeightsLoaded
    //!
    void setDeferredWeightsLoading(bool defer) noexcept
    {
        mImpl->setDeferredWeightsLoading(defer);
    }

    //!
    //! \brief Get the deferred-weight-loading flag.
    //!
    TRT_NODISCARD bool getDeferredWeightsLoading() const noexcept
    {
        return mImpl->getDeferredWeightsLoading();
    }
...
//! \class IRuntimeCache
//!
//! \brief A class for Runtime cache currently used for TensorRT JIT compilation.
//!        This cache can be serialized and deserialized to support persistence in disk files.
//!
...
public:
    ...
    //!
    //! \brief Serialize the Runtime cache to a stream.
    //!
    //! \return A IHostMemory object that contains the serialized Runtime cache.
    //!
    ...
    //!
    //! \brief Deserialize the Runtime cache from a buffer that contains a serialized Runtime cache.
    //!
    //! The blob is validated against the current environment first and rejected (leaving cache unchanged) on
    //! any mismatch. Following validation checks are performed:
    //! - the TensorRT-RTX version (exact match);
    //! - the GPU device properties / SKU (exact match);
    //! - the CUDA-context CiG (CUDA-in-Graphics) state (exact match);
    //! - the operating-system family (exact match);
    //! - the CUDA driver version (equal or newer than cached version. Major version must match);
    //!
    //! \param blob The serialized Runtime cache.
    //! \param size The size, in bytes, of the serialized Runtime cache buffer.
    //!
    //! \return True if the cache was compatible and its contents were successfully loaded; false otherwise (for
    //!         example, an invalid/corrupt blob or an environment mismatch), in which case this cache is left
    //!         unchanged.
    //!
    ...
    //!
    //! \brief Reset the Runtime cache. Clears all content within the cache.
    //!
    bool reset() noexcept
    {
        return mImpl->reset();
...
inline IRuntimeCache::~IRuntimeCache() noexcept = default;
...
//! Maximum number of dynamic shape specialization strategies in DynamicShapesKernelSpecializationStrategy enum. \see
//! DynamicShapesKernelSpecializationStrategy
template <>
struct impl::EnumMaxImpl<DynamicShapesKernelSpecializationStrategy>
{
    static constexpr int32_t kVALUE = 3;
};
...
//! Maximum number of CUDA graph strategies in CudaGraphStrategy enum. \see CudaGraphStrategy
template <>
struct impl::EnumMaxImpl<CudaGraphStrategy>
{
    static constexpr int32_t kVALUE = 2;
};
...
public:
    ...
    //!
    //! \brief Create an empty Runtime cache.
    //!
    //! \return A fresh IRuntimeCache object.
    //!
    ...
    //!
    //! \brief Set Runtime cache to the runtime config. Enables Runtime caching.
    //!
    //! \param cache The Runtime cache to set.
    //!
    ...
    //!
    //! \brief Get the Runtime cache from the runtime config.
    //!
    ...
    //!
    //! \brief Set the dynamic shape kernel specialization strategy for this config.
    //!
    //! \param dynamicShapesKernelSpecializationStrategy The dynamic shape specialization strategy.
    //!
    ...
    //!
    //! \brief Return the dynamic shape specialization strategy of this config.
    //!
    //! \return dynamicShapesKernelSpecializationStrategy The dynamic shape specialization strategy.
    //!
    ...
    //!
    //! \brief Set the strategy used for CUDA graphs for JIT (Just-In-Time) inference.
    //!        Default is kDISABLED.
    //!
    //! \note Even if the strategy was set to kWHOLE_GRAPH_CAPTURE, RTX CUDA graphs may be disabled
    //!       internally during runtime under following circumstances:
    ...
    //!        * The engine is streaming weights.
    //!       If CUDA graph is not supported for above or any other reason, it will silently ignore
    //!       the set strategy and run inference without CUDA graphs.
    //!
    //! \warning If the stream is being captured elsewhere, RTX CUDA graphs will be disabled.
    //!
    //! \param strategy The CUDA graph strategy.
    //!
    //! \return True if the strategy was within range and set successfully, false otherwise.
    //!
    ...
    //!
    //! \brief Return the strategy used for CUDA graphs for JIT (Just-In-Time) inference.
    //!
    //! \return The CUDA graph strategy.
    //!
...
public:
    ...
    //!
    //! \brief Check whether the engine's weights have been loaded onto the GPU.
    //!
    //! Returns true unless the engine was deserialized with IRuntime::setDeferredWeightsLoading(true)
    //! and ICudaEngine::loadWeights() or ICudaEngine::loadWeightsAsync() has not yet been called.
    //!
    //! \see IRuntime::setDeferredWeightsLoading, ICudaEngine::loadWeights, ICudaEngine::loadWeightsAsync
    //!
    TRT_NODISCARD bool areWeightsLoaded() const noexcept
    {
        return mImpl->areWeightsLoaded();
    }

    //!
    //! \brief Synchronously copy weights from the engine plan blob to the GPU.
    //!
    //! Used to complete a deferred deserialization (see IRuntime::setDeferredWeightsLoading).
    //! \p blob and \p size MUST refer to the same engine plan that was passed to
    //! IRuntime::deserializeCudaEngine. The caller keeps the buffer alive for the duration
    //! of this call only; the implementation uses an internal CUDA stream and synchronizes
    //! it before returning, so the buffer may be freed or reused once the call returns.
    //! After the call returns successfully, areWeightsLoaded() returns true. IExecutionContexts
    //! created before this call remain valid and may be used for inference after it returns; their
    //! internal weight bindings are refreshed automatically as part of this call.
    //!
    //! This method is not thread-safe: it mutates the engine's weight-load state and internal
    //! runtime resources, and must not be called concurrently with any other ICudaEngine method
    //! on the same engine (including areWeightsLoaded()).
    //!
    //! \param blob Pointer to the engine plan blob.
    //! \param size Size of the blob in bytes.
    //!
    //! \return true on success. Returns false if the engine was not deserialized in
    //! deferred mode, weights are already loaded, or the copy fails.
    //!
    TRT_NODISCARD bool loadWeights(void const* blob, int64_t size) noexcept
    {
        return mImpl->loadWeights(blob, size);
    }

    //!
    //! \brief Asynchronously copy weights from a stream to the GPU.
    //!
    //! Used to complete a deferred deserialization (see IRuntime::setDeferredWeightsLoading)
    //! while overlapping the weight transfer with other GPU work. \p streamReader MUST refer
    //! to the same engine plan that was passed to IRuntime::deserializeCudaEngine. The
    //! implementation does NOT synchronize \p stream before returning. The caller may issue
    //! subsequent ICudaEngine and IExecutionContext work (e.g. createExecutionContext,
    //! setInputTensorAddress, enqueueV3) on the same \p stream — they will be ordered after
    //! the weight transfer by CUDA stream semantics. A single cudaStreamSynchronize at the
    //! end of the pipeline is sufficient to wait for both the weight load and the inference
    //! work.
    //!
    //! Intended use case: overlap a large weight transfer (e.g. via a GDS-backed
    //! IStreamReaderV2) with other GPU work, or pipeline weight loading with the first
    //! inference call.
    //!
    //! After this call returns, areWeightsLoaded() returns true even though the GPU copy may
    //! still be in flight; the only stream-ordering guarantee is that anything submitted on
    //! \p stream after this call will see the loaded weights. IExecutionContexts created before
    //! this call remain valid and may be used for inference; their internal weight bindings are
    //! refreshed automatically as part of this call (any inference on them must, like all other
    //! work, be ordered after the weight transfer on \p stream).
    //!
    //! Work that is not stream-ordered after \p stream must be synchronized explicitly before it
    //! may observe the loaded weights. In particular, inference issued on a different stream, or via
    //! the synchronous executeV2(), must be preceded by a synchronization with \p stream (e.g.
    //! cudaStreamSynchronize(stream) or a cross-stream event); otherwise it may run against weights
    //! whose transfer has not completed.
    //!
    //! This method is not thread-safe: it mutates the engine's weight-load state and internal
    //! runtime resources, and must not be called concurrently with any other ICudaEngine method
    //! on the same engine.
    //!
    //! \param streamReader A stream from which TensorRT will read the engine plan's weight section.
    //!                     The stream's read() calls are issued on \p stream.
    //! \param stream       CUDA stream on which the async reads are issued, and on which
    //!                     subsequent inference work should also be queued so that it is
    //!                     ordered after the weight transfer.
    //!
    //! \return true if the async work was successfully enqueued. False indicates the call was
    //! rejected (engine not in deferred mode, weights already loaded, plan parse failed) and
    //! no async work was started on \p stream.
    //!
    //! \see IRuntime::setDeferredWeightsLoading, IStreamReaderV2
    //!
    TRT_NODISCARD bool loadWeightsAsync(IStreamReaderV2& streamReader, cudaStream_t stream) noexcept
    {
        return mImpl->loadWeightsAsync(streamReader, stream);
    }
